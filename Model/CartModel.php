...
class Cart {
    ...
    public function initializeCart(int $customerId): void {
        ...
        if ($cart) {
            ...
            $query = "INSERT INTO {$this->table} (customer_id) VALUES (?)";
            $stmt = $this->conn->prepare($query);
            $stmt->bind_param("i", $customerId);
            $stmt->execute();
    ...
    public function getTotal(): float {
        ...
        $query = "SELECT SUM(p.price * ci.quantity) AS total FROM {$this->itemsTable} ci JOIN products p ON ci.product_id = p.id WHERE ci.id = ?";
        $stmt = $this->conn->prepare($query);
    ...
    public function getItems(): array {
        $query = "SELECT  p.id, p.name, p.price, ci.quantity FROM {$this->itemsTable} ci JOIN products p ON ci.product_id = p.id WHERE ci.id = ?";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param("i", $this->cartId);
        $stmt->execute();
        $result = $stmt->get_result();
